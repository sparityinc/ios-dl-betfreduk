...
- (NSArray *)mapRepresentations:(id)representations atKeyPath:(NSString *)keyPath usingMapping:(RKMapping *)mapping
{
    ...
    if(objectsToMap != nil && [objectsToMap count] == 1 ){ //Jagadeesh HACK
        if( [[objectsToMap objectAtIndex:0] isKindOfClass:[NSArray class]] ){
            objectsToMap = [NSMutableArray arrayWithArray:[objectsToMap objectAtIndex:0]];
        }
    }
