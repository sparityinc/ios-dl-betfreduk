...
- (NSDictionary *)buildResponseMappingsDictionary
{
    ...
    //NSLog(@"%@",self.responseDescriptors);
    return dictionary;
}
...
- (RKMappingResult *)performMappingWithObject:(id)sourceObject error:(NSError **)error
{
    ...
    if (NSLocationInRange(self.response.statusCode, RKStatusCodeRangeForClass(RKStatusCodeClassSuccessful))) {
        if( [sourceObject isKindOfClass:[NSDictionary class]] && [[sourceObject objectForKey:@"response-status"] isEqualToString:@"success"]){ //Jagadeesh
            ...
            if( [sourceObject isKindOfClass:[NSDictionary class]] ){
                *error = [NSError errorWithDomain:RKErrorDomain code:RKMappingErrorUserDefined userInfo:@{@"message":[sourceObject objectForKey:@"response-message"]}];
            }
            else{
                *error = [NSError errorWithDomain:RKErrorDomain code:RKMappingErrorUserDefined userInfo:@{@"message":@"Please try again in a few minutes"}];
            }
